param(
    [string]$Prompt = 'Turn this into a simple watercolor postcard illustration while keeping the main subject recognizable.',
    [string]$InputImage = '',
    [string]$AspectRatio = '1:1',
    [string]$PicoclawHome = '',
    [string]$OutputDir = '',
    [string]$OutputPrefix = 'edit-smoke'
)
...
$defaultInputImage = Join-Path $repoRoot 'workspace\skills\generate-image\assets\momonga_refs_sheet.png'
...
function Resolve-InputImage {
    ...
    if (-not (Test-Path -LiteralPath $defaultInputImage -PathType Leaf)) {
        throw "Default input image not found: $defaultInputImage"
    }
    return (Resolve-Path -LiteralPath $defaultInputImage).Path
}
...
function New-OutputDirectory {
    ...
    $targetDir = Join-Path $baseDir ("edit-" + $timestamp)
...
$resolvedInputImage = Resolve-InputImage -ConfiguredPath $InputImage
$resolvedOutputDir = New-OutputDirectory -ConfiguredPath $OutputDir -HomePath $resolvedHome
$python = Get-PythonInvocation

$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ('picoclaw-image-edit-smoke-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($result.ExitCode -ne 0) {
        ...
        throw "Image edit smoke test failed with exit code $($result.ExitCode): $errorText"
    }

    if ([string]::IsNullOrWhiteSpace($result.Stdout)) {
        throw 'Image edit smoke test failed: helper returned empty stdout.'
    }
    ...
    if ($images.Count -eq 0) {
        throw 'Image edit smoke test failed: helper returned no image paths.'
    }
    ...
    if ($writtenFiles.Count -eq 0) {
        throw 'Image edit smoke test failed: helper reported images, but no output files were found.'
    }

    Write-Host 'Image edit smoke test succeeded.' -ForegroundColor Green
    Write-Host "Model:       $($parsed.model)"
    Write-Host "Provider:    $($parsed.provider)"
    Write-Host "Home:        $resolvedHome"
    Write-Host "Input image: $resolvedInputImage"
