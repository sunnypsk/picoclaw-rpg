...
func TestResolveRoute_AutoProvision_StrictOverridesBindings(t *testing.T) {
	agents := []config.AgentConfig{
		{ID: "main", Default: true},
		{ID: "telegram-bot"},
	}
...
func TestResolveRoute_AutoProvision_StrictRespectsChatTypes(t *testing.T) {
	agents := []config.AgentConfig{{ID: "main", Default: true}, {ID: "telegram-bot"}}
	bindings := []config.AgentBinding{{
		AgentID: "telegram-bot",
		Match: config.BindingMatch{
			Channel:   "telegram",
	...
	if route.AgentID != "telegram-bot" {
		t.Errorf("AgentID = %q, want 'telegram-bot' (strict mode should not apply to group here)", route.AgentID)
	}
	if route.MatchedBy != "binding.channel" {
		t.Errorf("MatchedBy = %q, want 'binding.channel'", route.MatchedBy)
	}
